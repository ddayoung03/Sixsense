...
// 1.5 -> 1.2: 너무 깐깐해서 유효한 프레임까지 자꾸 버려지면(=각도가 멈춘 것처럼
// 보이고, GET_ANGLE도 TDOA가 "아직 모름" 상태라 매번 타임아웃 대기) 반응이
// 느리고 부정확해 보인다. 완화해서 갱신 빈도를 늘린다.
#define PEAK_SIDELOBE_RATIO 1.2f
#define PSR_GUARD           3
// PHAT 백색화 지수: 1.0 = 완전 PHAT(가장 뾰족하나 잡음 민감), 0 = 순수 상호상관.
// 0.7 -> 0.9: 0.7은 피크를 너무 뭉툭하게 만들어서 PSR 게이트에 더 자주 걸리는
// 부작용이 있었다. 0.9로 올려 더 날카로운 피크 유지, 필요시 반향 심한 곳에서
// 다시 낮추면 됨.
#define PHAT_BETA           0.9f
